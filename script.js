...
// === TikTok Search UI Connector ===
document.addEventListener("click", async function(e){
  const btn = e.target.closest("[data-api-path]");
  if(!btn) return;

  const path = btn.getAttribute("data-api-path");

  // hanya trigger untuk tiktok search
  if(!path.includes("/api/tiktok/search")) return;

  const keyword = prompt("Masukkan keyword TikTok:");
  if(!keyword) return;

  const apikey = prompt("Masukkan APIKEY:");
  if(!apikey) return;

  const url = `${path}${encodeURIComponent(keyword)}&apikey=${apikey}`;

  try{
    showToast("Loading TikTok Search...", "info");
    ...
    win.document.write(`
      <html>
      <head>
        <title>TikTok Search Result</title>
        <style>
          body{background:#0f0f0f;color:white;font-family:Arial;padding:20px}
          .card{background:#151515;padding:12px;border-radius:10px;margin-bottom:10px}
      ...
      <body>
        ...
        ${
          data.result?.result?.map(v=>`
            <div class="card">
              <b>${v.title || "No title"}</b><br>
              <small>@${v.author || "-"}</small><br><br>
              ${v.play ? `<video src="${v.play}" controls></video>` : ''}
            </div>
          `).join("") || "No result"
        }
      </body>
    ...
    showToast("TikTok Search Success", "success");

  }catch(err){
    console.error(err);
    showToast("Error TikTok Search", "error");
  }
});
